...
<div class="topnav" id="myTopnav">
  ...
  <a href="#news" class="active">Add Timesheet</a>
...
$sta = $connection->prepare($check_query);
$sta->execute();
...
if($sa)
{
  ...
  $ts_query = "insert into student_entry(User_ID, FromTime, ToTime, ADate,SubName, StaffName, ClassType, Remarks, Others)
              values ($ts_uid,'$ts_tfrom', '$ts_tto', '$ts_dat', '$ts_sub', 'anys' ,'$ts_ctype', '$ts_remark','none')";
  
              $str = $connection->prepare($ts_query);
              $str->execute();
              $sd = $str->fetch(PDO::FETCH_ASSOC);
              //$ts_res = $connection->prepare($ts_query);
              //$stmt->execute();
  ...
  {
  echo "<p style='margin-left: 150px;'><strong>Added Successfully</strong></p>";
  }
}
